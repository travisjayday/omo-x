...
private:
    ...
    QPoint winPos;
    std::vector<cv::Point> previousPos;
    int maxPrevPosSize =  5;
    int currentPos = 0;
    int programIdx = 0;
    cv::Point previousDot = cv::Point(0, 0);
    ...
    void PGRM_detectFaces(cv::Mat &input);
...
public:
     ...
     * @param height Height of the camera image
     */
    CameraThread(BetterVideoCapture* camera, QVideoFrame* videoFrame,
                 unsigned char* cvImageBuf, int width, int height,
                 bool _fingerDetection, QPoint winPos, int programIdx);
...
private:
    ...
    NetworkCameraTask* task = NULL;            ///< The camera loop method and parameter container
